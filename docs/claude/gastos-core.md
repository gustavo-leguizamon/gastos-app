...
- `verificado` (`verificado`, default `false`) — para revisar sub-items uno a uno y marcar cuáles están correctos. En `GastoItemDialog`, cada fila tiene un toggle (icono `CheckCircleIcon` verde si verificado / `RadioButtonUncheckedIcon` naranja si no), y la fila se pinta con fondo verde tenue (verificado) o naranja tenue (no verificado) para tener a la vista los pendientes de revisar. Se togglea vía `PATCH { verificado }`. El `PUT` (editar item completo) preserva `verificado` enviándolo en el payload — el `EditState` lo incluye. La lista se puede filtrar por este estado (**Todos / Verificados / Sin verificar**, con el conteo de cada uno) para revisar resúmenes largos sin perder de vista lo que falta — ver el layout de `GastoItemDialog` más abajo.
...
**UI:** `GastoForm` y `GastoItemDialog` usan `AppSelect` (categoría única, emptyLabel "Sin categoría") + `AppMultiSelect` (etiquetas, recortadas según la categoría elegida — ver [Etiquetas sugeridas por categoría](#etiquetas-sugeridas-por-categoría)), ambos con `onCreate` para alta inline. `GastosTable` muestra categoría (color primary) + etiquetas (🏷️) en grilla/cards/sub-ítems; la búsqueda incluye ambas. La **edición masiva** (`BulkAccionesBar` + `PATCH /api/gastos/categorias` y `/api/gastos/etiquetas`) permite asignar/limpiar la categoría única y agregar/quitar una etiqueta a varios gastos a la vez (ver [Acciones masivas](#acciones-masivas-clasificación--borrado)). ABM de ambas en `/configuracion`: cada ítem muestra un chip con la cantidad de usos y el botón de borrar queda **deshabilitado** cuando la categoría/etiqueta está en uso (mismo patrón que Conceptos). En toda la pantalla de configuración (casas, monedas, categorías, etiquetas, tarjetas, conceptos y cierres de tarjeta) el borrado pide confirmación con `ConfirmDialog` antes de ejecutarse.

### Etiquetas sugeridas por categoría

El multiselect de etiquetas ofrecía **todas** las etiquetas del sistema para cualquier gasto (35 en la base real), cuando el set relevante por categoría es de 1 a 11: `Delivery` no comparte nada con `Celular`. Al elegir la categoría, el control ahora arranca recortado a las etiquetas acordes a esa categoría.

**El criterio se deriva del histórico, no se configura.** Módulo puro `src/lib/etiquetas-sugeridas.ts`:
- `computeSugerencias(usos, minCategorias?, reglas?)` agrega las filas `{ etiquetaId, categoriaId }` en `{ transversales: number[], por_categoria: Record<categoriaId, number[]>, reglas: ReglaEtiqueta[] }`. Cada lista va ordenada por uso descendente y desempata por id, para que el dropdown no se reordene entre cargas.
- `MIN_CATEGORIAS_TRANSVERSAL = 3` — desde cuántas categorías distintas una etiqueta deja de ser propia de una categoría y pasa a ofrecerse en todas. Es la constante a tocar si el recorte queda flojo o apretado.
- `etiquetasSugeridas(sugerencias, categoriaId)` devuelve, en orden: las **fijadas** a mano, las propias de la categoría y las transversales que falten, sacando las **excluidas**. Devuelve **`null`** cuando no hay criterio para recortar —sin sugerencias cargadas, sin categoría elegida, o cuando no hay nada que sugerir y ninguna regla lo explica (base recién estrenada)— y en ese caso la UI muestra todas. `[]` **sí** es un recorte y se respeta: es lo que queda cuando se excluyeron a mano todas las candidatas, y ahí el vacío es una decisión, no falta de datos.
- `reglasDeCategoria` / `origenEtiqueta` / `parseReglasBody` — las tres al servicio del ABM (ver abajo).

**Por qué derivado y no una whitelist por categoría mantenida a mano:** las etiquetas más usadas son justamente las transversales — `Mercado Libre` aparece en 10 categorías, `Mercado Pago` en 8, `PedidosYa` en 4 con 318 usos. Habría que declararlas en casi todas las categorías, y la que se olvide termina retipeada como variante del mismo nombre: el eje partido en dos que `resolveEtiqueta` existe para evitar. Además una etiqueta nueva quedaría invisible hasta que alguien la habilite. Derivándolo no hay mantenimiento y se aprende solo: la etiqueta que se crea inline queda asociada a la categoría por el solo hecho de haberse usado ahí. El costo es el arranque en frío — una categoría sin histórico recibe sólo las transversales.

**Endpoint:** `GET /api/etiquetas/sugeridas` devuelve el mapa completo (todas las categorías, sólo ids) más las reglas manuales. Se pide una vez junto con el resto de los catálogos, así cambiar la categoría en el form no dispara otro fetch ni deja el dropdown en loading. Excluye los gastos `esTarjeta` (contenedores; la clasificación real está en sus sub-items, que sí entran). Lleva **`export const dynamic = 'force-dynamic'`**: el handler no recibe `req` ni lee headers, así que Next lo prerenderizaría en el build y devolvería para siempre el mapa que existía al compilar — las etiquetas usadas después no se ofrecerían nunca y el ABM parecería no guardar nada.

**UI:** `GastoForm` (`destacadas` desde la `categoria_id` que observa) y `GastoItemDialog` (alta y edición inline, cada uno con la categoría de **su** sub-item, que no se hereda del gasto padre). El recorte lo implementa la prop `destacadas` de `AppMultiSelect` y es **blando**: fila "Ver todas (N)" al pie del listado, y al tipear se busca sobre todas las etiquetas — si la búsqueda respetara el recorte, una etiqueta existente pero oculta no aparecería y se ofrecería "Agregar «X»", creando el duplicado. Los filtros de la grilla y de reportes **no** se recortan: ahí se filtra por etiqueta sin categoría de referencia. Si el fetch falla, se ofrecen todas. Tests: `etiquetas-sugeridas.test.ts` y `etiquetas/sugeridas/route.test.ts`.

#### Excepciones manuales y ABM

Sobre lo derivado hay una capa de correcciones a mano, para los dos casos que el histórico no puede resolver solo: la **categoría nueva** que todavía no tiene datos, y el **uso puntual mal clasificado** que quedó contaminando la lista para siempre.

**Modelo `CategoriaEtiquetaRegla`** (`categoriaId`, `etiquetaId`, `modo`, unique `(categoriaId, etiquetaId)`, `onDelete: Cascade` en las dos puntas; migración `20260821100000_categoria_etiqueta_regla`). `modo` es `'fijar'` (ofrecerla acá aunque el histórico no la respalde) o `'excluir'` (no ofrecerla nunca acá, ni siquiera si es transversal). **Excluir gana sobre fijar y sobre el histórico**, y las fijadas van primero en el dropdown porque son una decisión explícita, más fuerte que cualquier frecuencia.

Es una capa de **excepciones, no la fuente de verdad**: la ausencia de fila es el caso normal y la tabla arranca (y normalmente sigue) vacía. Las reglas no cuentan como `uso` de la categoría/etiqueta, así que no bloquean su borrado — se van por cascade.

**API:** `GET /api/categorias/[id]/etiquetas` (las reglas de esa categoría) y `PUT` con `{ fijar: number[], excluir: number[] }`, que **reemplaza todas** las reglas de la categoría. Es reemplazo y no alta/baja por par porque el ABM edita una categoría entera en un diálogo: mandar el estado final evita diffear en el cliente y hace que reintentar el guardado sea idempotente; listas vacías borran las reglas y vuelven a lo derivado. Valida con `parseReglasBody` (dedup, ids enteros, y rechaza la etiqueta que viene fijada **y** excluida — sería una regla contradictoria y el unique sólo admitiría una fila). Las etiquetas se verifican con un `count` antes de escribir y no contra el FK, porque el borrado ya se ejecutó cuando el `create` falla dentro de la transacción. Test: `categorias/[id]/etiquetas/route.test.ts`.

**UI:** `EtiquetasPorCategoria` (`src/components/configuracion/EtiquetasPorCategoria.tsx`), acordeón **"Etiquetas por categoría"** en `/configuracion`. Una fila por categoría con el conteo (`N de 35`) y los chips de lo que ofrece, **cada uno mostrando de dónde sale**: ya usada acá (outlined primary), transversal (atenuado), fijada a mano (filled + 📌) y excluida (tachado + 🚫). Esa distinción es el punto: la tabla no lista relaciones guardadas — la mayoría de los pares que muestra no existen en ninguna fila de la base. El diálogo de edición son dos `AppMultiSelect` (fijar / no ofrecer) que se excluyen entre sí en el cliente para no mandar un par que el backend rechazaría, más un **preview** de cómo queda la lista calculado con la misma `etiquetasSugeridas` que usa el form, y un "Limpiar reglas" para volver a lo puramente derivado.
...
`GastoItemDialog` layout 2-col (`maxWidth="md"`, height 90vh): izq (340px) resumen + add form; der lista scrollable. Overflow independiente. La columna derecha tiene, arriba de la lista, un buscador (`filtroItems`) y un **filtro por marca de verificado** (`filtroVerificado`: `todos` / `verificados` / `pendientes`, `ToggleButtonGroup` con el conteo de cada estado). Los dos se combinan con AND en **`matchSubitem(item, busqueda, verificado)`** (`src/lib/gastos-filtro.ts`, puro y testeado): la búsqueda matchea descripción, categoría y etiquetas del sub-item (case-insensitive) y `verificado` ausente/`null` cuenta como pendiente. El resumen de la izquierda y los conteos del toggle se calculan sobre **todos** los items, no sobre los filtrados.

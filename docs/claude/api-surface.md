...
| `GET /api/tarjetas/proximos-cierres` | Las tarjetas **vigentes** en el `(mes, anio)` consultado (las dadas de baja desde ese mes o antes no salen) con el estado de su ciclo, ordenadas por el cierre que tienen por delante, asc (las que no tienen ninguna fecha, al final). Params: `mes`, `anio`, `today` (YYYY-MM-DD). Returns `{ id, nombre, banco, marca, banco_logo, banco_icono, fecha_cierre, fecha_vencimiento, fecha_proximo_cierre, estado: 'cerrado' \| 'abierto' \| 'por_cerrar' \| 'sin_fecha', dias_para_cierre, progreso }[]`. |

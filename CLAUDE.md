...
| `src/lib/vencimientos.ts` (`vencePorGasto`) | Si un gasto vence por su propia `fechaVencimiento` (sin sub-items **o** resumen de tarjeta) o por sus sub-items marcados `incluyeEnVencimiento`. Compartida por `computeResumen` (`pagar_hoy`) y `VencimientosHoyAlert` para que la card y el alert no se desincronicen. | `vencimientos.test.ts` |

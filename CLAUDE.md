...
| `src/lib/vencimientos.ts` (`vencePorGasto`, `vencimientosDelDia`) | `vencePorGasto`: si un gasto vence por su propia `fechaVencimiento` (sin sub-items **o** resumen de tarjeta) o por sus sub-items marcados `incluyeEnVencimiento`. Compartida por `computeResumen` (`pagar_hoy`) y `VencimientosHoyAlert` para que la card y el alert no se desincronicen. `vencimientosDelDia`: la lista de entradas (gasto o sub-item) que vencen hoy y siguen impagas, con su monto — compartida por `VencimientosHoyAlert` y el push diario (`/api/cron/vencimientos`). | `vencimientos.test.ts` |
